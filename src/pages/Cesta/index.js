...
export default function Cesta() {
  ...
  useEffect(() => {
    function atualizaTotal() {
      let newSubtotal = 0;
      basket.forEach((item) => {
        let itemTotal = parseFloat(item.VR_UNITARIO);
        if (item.VR_ACRESCIMOS) {
          item.VR_ACRESCIMOS.forEach((extra) => {
            itemTotal += parseFloat(extra.VR_UNITARIO);
          });
        }
        newSubtotal += itemTotal;
      });
      setSubtotal(newSubtotal);
      const soma = newSubtotal + parseFloat(delivery?.TAXA_ENTREGA);
      setTotal(soma);
    }
    atualizaTotal();
  }, [basket, delivery, setSubtotal]);
